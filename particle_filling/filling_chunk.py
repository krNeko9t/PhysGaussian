...
# Prevents pathological covariances from making densify explode to hours.
...
def densify_grids(
    ...
    for pi in range(init_particles.shape[0]):
        ...
        # Guard against out-of-bounds access (can happen due to numeric drift).
        if (
            0 <= i
            and i < grid.shape[0]
            and 0 <= j
...
def densify_grids_range(
    ...
    # Same as densify_grids, but only processes particles in [start, end).
...
def fill_dense_grids_range(
    ...
    # Same as fill_dense_grids, but only processes flattened grid indices in [start, end).
    new_start_idx = start_idx
...
def internal_filling_range(
    ...
    # Same as internal_filling, but only processes flattened grid indices in [start, end).
    # This enables chunked execution with a progress bar.
    new_start_idx = start_idx
    n = grid.shape[0]
    nn = n * n
    for t in range(start, end):
...
def fill_particles(
    ...
    if progress and n_particles > particle_chunk_size:
        ...
        densify_grids(
            ti_pos, ti_opacity, ti_cov, grid, grid_density, grid_dx, densify_r_cap
        )
        if sync_each_chunk:
            ti.sync()
    ...
    if fill_num >= max_samples:
        print(
            f"[particle_filling] WARNING: filled particles ({fill_num}) exceed max_particles_num ({max_samples}); clamping to avoid out-of-bounds."
        )
        fill_num = max_samples
